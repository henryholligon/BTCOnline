...
export default function MerchantCard({ merchant }: MerchantCardProps) {
  ...
  const getIconBgColor = () => {
    ...
    if (name === 'oshi good') return 'bg-white shadow-none';
            ...
            {merchant.logo.startsWith("/") || merchant.logo.startsWith("http") ? (
              <img 
                ...
                className={`w-full h-full object-contain ${
                  ...
                  merchant.name.toLowerCase() === 'bramleigh farm' ? 'scale-110 object-cover' : 
                  merchant.name.toLowerCase() === 'oshi good' ? 'scale-90' : ''
                }`} 
              />
            ) : (
              merchant.logo
