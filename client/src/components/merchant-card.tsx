                ...
                <Popover>
                  <PopoverTrigger asChild>
                   ...
                   <PopoverContent className="w-56 p-2" align="center" side="top">
                    <button
                       className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md text-sm hover:bg-muted transition-colors text-left"
                    ...
                    >
                     ...
                     <div className="border-t border-border/50 mt-1.5 pt-2">
                       <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70 px-1 mb-1.5">
                       ...
                       {!relayLikesLoaded && likeAuthorsList.length === 0 ? (
                         ...
                         <div className="space-y-1.5 max-h-40 overflow-y-auto">
                           {likeAuthorsList.map(author => {
                             ...
                             return (
                               <div key={author.pubkey} className="flex items-center gap-2 px-1">
                                 <div className="h-5 w-5 rounded-full bg-muted flex items-center justify-center shrink-0 text-[9px] font-bold text-muted-foreground uppercase overflow-hidden">
                                   {profile?.picture
                                     ? <img src={profile.picture} alt={displayName} className="h-full w-full object-cover" onError={e => { (e.currentTarget as HTMLImageElement).style.display = "none"; (e.currentTarget.parentElement as HTMLElement).textContent = displayName.slice(0, 1); }} />
                                     : displayName.slice(0, 1)}
                                 </div>
                                 <Link href={`/profile/${author.npub}`} className="text-xs font-medium truncate hover:text-primary transition-colors" title={author.npub}>{displayName}</Link>
                               </div>
                             );
                           })}
                         </div>
                       ) : (
                         <p className="text-xs text-muted-foreground px-1">No public likes yet.</p>
                       )}
                     </div>
                  </PopoverContent>
                </Popover>
